"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TransferResolver = void 0;
const graphql_1 = require("@nestjs/graphql");
const common_1 = require("@nestjs/common");
const transfer_service_1 = require("./transfer.service");
const transfer_input_1 = require("./dto/transfer.input");
const transaction_model_1 = require("../transaction/models/transaction.model");
const wallet_model_1 = require("../wallet/models/wallet.model");
const gql_auth_guard_1 = require("../common/guards/gql-auth.guard");
const current_user_decorator_1 = require("../common/decorators/current-user.decorator");
let TransferResolver = class TransferResolver {
    constructor(transferService) {
        this.transferService = transferService;
    }
    async transfer(user, input) {
        return this.transferService.transfer(user.id, input);
    }
    async deposit(user, amount) {
        return this.transferService.deposit(user.id, amount);
    }
};
exports.TransferResolver = TransferResolver;
__decorate([
    (0, graphql_1.Mutation)(() => transaction_model_1.TransactionType, { description: 'Transfer money to another user by their mobile number' }),
    (0, common_1.UseGuards)(gql_auth_guard_1.GqlAuthGuard),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __param(1, (0, graphql_1.Args)('input')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, transfer_input_1.TransferInput]),
    __metadata("design:returntype", Promise)
], TransferResolver.prototype, "transfer", null);
__decorate([
    (0, graphql_1.Mutation)(() => wallet_model_1.WalletType, { description: 'Add dummy funds to your wallet (demo only)' }),
    (0, common_1.UseGuards)(gql_auth_guard_1.GqlAuthGuard),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __param(1, (0, graphql_1.Args)('amount', { type: () => graphql_1.Float })),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Number]),
    __metadata("design:returntype", Promise)
], TransferResolver.prototype, "deposit", null);
exports.TransferResolver = TransferResolver = __decorate([
    (0, graphql_1.Resolver)(),
    __metadata("design:paramtypes", [transfer_service_1.TransferService])
], TransferResolver);
//# sourceMappingURL=transfer.resolver.js.map